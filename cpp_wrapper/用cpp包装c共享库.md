# 任务:用cpp包装c共享库得到cpp共享库

## 提示词

### 1\. 初始化

你将执行一项代码转换任务,定义任务为 将注释说明的c的函数声明转化为对应的cpp函数声明，比如将
`// extern void set_print(CB_S print); // extern void set_group_listener(CB_I_S cCallback); // extern void set_conversation_listener(CB_I_S cCallback);`
转化为:
...
```
对于返回的char类型,转化为使用std::string类型,对于传入的char参数类型,改为const std::string&类型
```
